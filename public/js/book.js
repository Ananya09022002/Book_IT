...
const loadUserData = (data) => {
  ...
  if (!data.bookAccess.includes(bookId)) {
    ...
    document.getElementById(
      "main-btn"
    ).innerHTML = `<button id="mainBook" onclick="purchaseBook(${data.coins})">PURCHASE THIS BOOK</button>`;
  } else {
    document.getElementById(
      "main-btn"
    ).innerHTML = `<button id="mainBook" onclick="viewBook()">READ THIS BOOK</button>`;
  }
};
